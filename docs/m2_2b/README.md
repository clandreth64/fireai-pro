...
| G | 9.4.3.1 | `sprinkler.response_type` | `fact_requirement` on `sprinkler.response`: the listing's stated response type must be one of the allowed values YOU supply (M2.2B.2) | in | — |
...
**Rule G and installation context (M2.2B.2).**
- G is a *fact requirement*. It checks the listing's `response_type`; it does not measure geometry.
- Its applicability includes `installation.context = new_system`.
- It covers a NEW Light Hazard standard-spray installation. Existing-system exception paths and other
  sprinkler technologies are outside the envelope; list them under `exceptions_outside_envelope`.
- The envelope requires:
  - an explicit installation context of `new_system`;
  - a known listing response type;
  - an approved rule that evaluates it.
- A missing response type refuses. A value outside G's allowed set fails every layout.

**What an approved package means.** Approving A–G yields at most:

```
RULE_APPROVAL:               APPROVED_FOR_INTERNAL_R_AND_D_RULE_VALIDATION
NFPA_RULE_SUBSET_STATUS:     VALIDATED
NFPA_ENVELOPE_COMPLETENESS:  NOT_ESTABLISHED
NFPA_COMPLIANCE:             NOT_CLAIMED
EXTERNAL_RELEASE:            NOT_RELEASE_ELIGIBLE
```

The development completeness manifest (`NFPA13-2019-DEV-ENVELOPE-1-MANIFEST`) lists A–G as the first
*intended* subset. Whether they are ALL the requirements for the envelope is for a later qualified code
review; approving it needs a distinct author, reviewer and approver plus a written review statement.
Even then, NFPA compliance stays NOT_CLAIMED.
