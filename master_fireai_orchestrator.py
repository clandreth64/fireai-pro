...
# =============================================================================
# ENHANCED EXPORT GENERATORS (ALL ORIGINAL FUNCTIONALITY)
# =============================================================================
...
Generated by FireAI Pro Master Pipeline Orchestrator v4.1.1
"""
...
"""
    ...
    async def _generate_reportlab_pdf(self, context: PipelineContext, output_path: Path, report_type: str, logger):
        ...
        story.append(Paragraph("Generated by FireAI Pro Master Pipeline Orchestrator v4.1.1", styles['Normal']))
...
# =============================================================================
# FALLBACK DATA GENERATORS (ALL ORIGINAL FUNCTIONALITY)
# =============================================================================
...
try:
    ...
    print("* Master orchestrator with ChatGPT enhancements initialized")
except Exception as e:
...
app = FastAPI(
    title="FireAI Pro Master Production System with ChatGPT Enhancements",
    description="Production-ready enterprise fire sprinkler design orchestrator with comprehensive artifact publishing and ChatGPT context tracking improvements",
    version="4.1.1",
...
class PipelineRequest(BaseModel):
    ...
    zip_code: Optional[str] = Field(default=None, regex=r'^\d{5}(-\d{4})?#!/usr/bin/env python3
"""
...
artifact publishing capabilities and ChatGPT context tracking improvements.
...
- ChatGPT Block A/B pattern implementation

Author: FireAI Pro Team
Version: 4.1.1 Production (ChatGPT Enhanced)
License: Proprietary
"""
...
class MasterOrchestrator:
    """Production-ready master orchestrator with artifact publishing and ChatGPT improvements"""
    ...
    def __init__(self, settings):
        ...
        # ChatGPT Enhancement: Context tracking per job
        ...
        self.logger.info("Master orchestrator initialized", extra={"version": "4.1.1"})
    ...
    async def run_for_project_id(self, project_id: str, job_id: str):
        ...
        try:
            ...
            # ===== Block A: create ctx & remember common inputs =====
            ...
            # Figure out where the uploaded plan is (work even if you don't know the var name)
            upload_guess = None
            for name in ("upload.pdf", "upload.dwg", "upload.dxf"):
                cand = project_dir / name
                if cand.exists():
                    upload_guess = cand
                    break
            
            if upload_guess is None:
                # fallback: first PDF in the project dir
    ...
    async def _execute_pipeline_with_chatgpt_enhancements(self, job_id: str, project_data: Dict, 
        ...
        """Execute pipeline with ChatGPT enhancements for context tracking"""
        ...
        try:
            ...
            # ===== Block B: discover outputs, publish, and attach deliverables =====
            ...
            # 1) DXF / IFC by conventional filenames
            _set_if_exists("design_dxf_path", pdir / "design.dxf")
            _set_if_exists("ifc_path", pdir / "model.ifc")

            # 2) Hydraulics report (PDF first, else TXT)
            hyd = list(pdir.glob("hydraulics*.pdf")) or list(pdir.glob("hydraulics*.txt"))
            if hyd:
                self.ctx["hydraulics_report_pdf"] = str(hyd[0])

            # 3) Compliance report (PDF/TXT)
            cmp = list(pdir.glob("compliance*.pdf")) or list(pdir.glob("compliance*.txt"))
            if cmp:
                self.ctx["nfpa_compliance_pdf"] = str(cmp[0])

            # 4) Seismic bracing report (PDF/TXT)
            br = list(pdir.glob("bracing*.pdf")) or list(pdir.glob("bracing*.txt"))
            if br:
                self.ctx["seismic_bracing_pdf"] = str(br[0])

            # 5) Multi-standard report (PDF/TXT)
            ms = list(pdir.glob("multi*standard*.pdf")) or list(pdir.glob("multi*standard*.txt"))
            if ms:
                self.ctx["multi_standard_pdf"] = str(ms[0])

            # 6) BOM (CSV/XLSX/PDF)
            bom_csv = list(pdir.glob("bom*.csv"))
            bom_xlsx = list(pdir.glob("bom*.xlsx"))
            bom_pdf = list(pdir.glob("bom*.pdf"))
            if bom_csv:
                self.ctx["bom_csv"] = str(bom_csv[0])
            if bom_xlsx:
                self.ctx["bom_xlsx"] = str(bom_xlsx[0])
            if bom_pdf:
                self.ctx["bom_pdf"] = str(bom_pdf[0])

            # 7) Optional diagnostics
            trace = list(pdir.glob("routing*.json"))
            log = list(pdir.glob("engine*.txt")) + list(pdir.glob("*.log"))
            if trace:
                self.ctx["routing_trace_json"] = str(trace[0])
            if log:
                self.ctx["engine_log_txt"] = str(log[0])

            # 8) Publish artifacts into /data/projects/<id>/ and update job deliverables
            manifest = publish_artifacts(self.ctx, project_dir_str)
...
async def run_master_pipeline(
    ...
    try:
        ...
        return {
            "project_id": project_id,
            "status": "submitted",
            "message": "Master pipeline processing initiated with ChatGPT enhancements",
            "request_id": request_id,
            "idempotency_key": idempotency_key,
    ...
    try:
        ...
        return FileResponse(
            ...
            headers={
                ...
                "X-FireAI-Version": "4.1.1",
                "X-ChatGPT-Enhanced": "true"
            }
        )
...
@app.get("/")
async def root():
    """Enhanced API information with ChatGPT improvements and artifact publishing details"""
    engine_summary = {
    ...
    return {
        ...
        "version": "4.1.1",
        "status": "operational",
        "description": "Production-ready enterprise fire sprinkler design pipeline with comprehensive artifact publishing and ChatGPT context tracking enhancements",
        
        "chatgpt_enhancements": {
            "block_a_pattern": "Enhanced context initialization with automatic upload discovery",
            "block_b_pattern": "Intelligent artifact discovery using glob patterns and fallback handling",
            "improved_context_tracking": "Per-job context dictionary for comprehensive artifact management",
            "enhanced_file_discovery": "Automatic pattern matching for generated files",
            "better_manifest_generation": "Comprehensive metadata with file categorization and descriptions"
        },
        ...
        "enterprise_features": {
            ...
            "chatgpt_context_improvements": True
        },
        ...
        "artifact_publishing": {
            ...
            "chatgpt_enhanced_discovery": True
        },
        ...
        "configuration": {
            ...
            "chatgpt_enhancements_enabled": True,
            "rate_limits": {
...
# =============================================================================
# LEGACY API SUPPORT (PRESERVED FROM ORIGINAL)
# =============================================================================
...
def main():
    ...
    print("=" * 70)
    print("FireAI Pro Master Production Orchestrator v4.1.1")
    print("WITH CHATGPT CONTEXT TRACKING ENHANCEMENTS")
    print("=" * 70)
    print("* ChatGPT Enhancements:")
    print("  * Block A: Enhanced context initialization and upload discovery")
    print("  * Block B: Automatic artifact discovery and publishing")
    print("  * Improved file pattern matching with glob patterns")
    print("  * Enhanced manifest generation with comprehensive metadata")
    print("  * Better error handling and logging throughout pipeline")
    print("  * Per-job context tracking for improved artifact management")
    print()
    ...
    print(f"  ChatGPT Enhancements: ENABLED")
    print()
    ...
    print(f"   ChatGPT Enhancements: {'Enabled' if health.get('chatgpt_enhancements') else 'Disabled'}")
    print()
    ...
    print("* Starting enhanced production server with ChatGPT improvements...")
    print("=" * 70)
