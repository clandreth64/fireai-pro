...
class FireAIProMasterOrchestrator:
    ...
    async def process_project(self, 
        ...
        try:
            ...
            # === PUBLISH ARTIFACTS (AFTER exports; PROJECT folder) ===
try:
    import json, shutil
    from pathlib import Path
    ...
    # Map your internal keys -> public canonical filenames expected by clients
    src_map = {
        ...
        "compliance.pdf":    (result.export_files.get("compliance.pdf")    or result.export_files.get("compliance_pdf")),
        "hydraulics.pdf":    (result.export_files.get("hydraulics.pdf")    or result.export_files.get("hydraulics_pdf")),
        "bom.pdf":           (result.export_files.get("bom.pdf")           or result.export_files.get("bom_pdf")),
        "bracing.pdf":       (result.export_files.get("bracing.pdf")       or result.export_files.get("bracing_pdf")),
        "multistandard.pdf": (result.export_files.get("multistandard.pdf") or result.export_files.get("multistandard_pdf")),
    }

    artifacts = []

    # Keep original upload for convenience
    up = (Path(job_dir) / "upload.pdf")
    if up.exists():
        shutil.copy2(up, publish_dir / "upload.pdf")
        artifacts.append({"name": "upload.pdf", "path": "upload.pdf"})

    # Publish outputs: copy local files, preserve URLs
    for public_name, src in src_map.items():
        if not src:
            continue
        s = str(src)
        try:
            if (s.startswith("/") or (len(s) > 2 and s[1:3] == ":\\" ) or s.startswith(".")) and Path(s).exists():
                shutil.copy2(s, publish_dir / public_name)
                artifacts.append({"name": public_name, "path": public_name})
            elif s.startswith("http"):
                artifacts.append({"name": public_name, "url": s})
        except Exception as e:
            job_logger.warning(f"publish {public_name} failed: {e}")

    # Write manifest used by /artifacts endpoint
    (publish_dir / "artifacts.json").write_text(json.dumps({"artifacts": artifacts}, indent=2), encoding="utf-8")

    # Optional: tiny project.json (handy for /download tooling)
    (publish_dir / "project.json").write_text(json.dumps({
        "project_id": proj_id,
        "job_id": result.job_id,
        "status": getattr(result.status, "value", str(result.status))
    }, indent=2), encoding="utf-8")

    # Surface canonical names back to callers (helpful if you also return deliverables)
    for a in artifacts:
        if "url" in a:
            result.export_files[a["name"]] = a["url"]
        else:
            result.export_files[a["name"]] = str(publish_dir / a["path"])

    job_logger.info(f"📦 Published {len(artifacts)} artifacts → {publish_dir}")
except Exception as e:
