...
def orchestrate(project_dir: Path, output_dir: Path) -> dict:
    """
    Minimal, safe placeholder orchestrator.
    Writes all expected deliverables into output_dir so your client can download them.
    Replace the placeholder generation with your real pipeline when ready.
    """
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    pdir = Path(project_dir)

    # 1) Find the uploaded plan and copy it into the job folder (nice for debugging)
    candidates = list(pdir.glob("*.dwg")) + list(pdir.glob("*.dxf")) + [pdir / "upload.pdf"]
    src = next((c for c in candidates if c.exists()), None)
    if src:
        shutil.copy2(src, output_dir / src.name)

    # 2) Produce placeholders for all expected outputs
    # DXF (tiny valid-ish file)
    (output_dir / "design.dxf").write_text("0\nSECTION\n2\nENTITIES\n0\nENDSEC\n0\nEOF\n", encoding="ascii")

    # CSVs
    (output_dir / "materials.csv").write_text("item,qty\nsprinkler_head,12\npipe,200ft\nhanger,34\n", encoding="utf-8")
    (output_dir / "bom.csv").write_text("part,qty\ntee,8\nelbow,14\nvalve,2\n", encoding="utf-8")

    # PDFs (minimal stubs)
    pdf_stub = b"%PDF-1.4\n1 0 obj<<>>endobj\ntrailer<<>>\n%%EOF\n"
    (output_dir / "compliance.pdf").write_bytes(pdf_stub)
    (output_dir / "hydraulics.pdf").write_bytes(pdf_stub)
    (output_dir / "bom.pdf").write_bytes(pdf_stub)
    (output_dir / "bracing.pdf").write_bytes(pdf_stub)
    (output_dir / "multistandard.pdf").write_bytes(pdf_stub)

    # IFC (very small placeholder)
    (output_dir / "model.ifc").write_text("ISO-10303-21;\nEND-ISO-10303-21;\n", encoding="ascii")

    # Optional: write a tiny job-level manifest for human reading (your app writes the API manifest)
    summary = {
        "note": "placeholder deliverables generated by orchestrate()",
        "inputs": src.name if src else None,
    }
    (output_dir / "artifacts.json").write_text(json.dumps(summary, indent=2))

    # Important: return mapping of deliverable name -> ABSOLUTE path (your FastAPI run() converts to relative)
    return {
        "design.dxf": str((output_dir / "design.dxf").resolve()),
        "materials.csv": str((output_dir / "materials.csv").resolve()),
        "compliance.pdf": str((output_dir / "compliance.pdf").resolve()),
        "model.ifc": str((output_dir / "model.ifc").resolve()),
        "hydraulics.pdf": str((output_dir / "hydraulics.pdf").resolve()),
        "bom.pdf": str((output_dir / "bom.pdf").resolve()),
        "bracing.pdf": str((output_dir / "bracing.pdf").resolve()),
        "multistandard.pdf": str((output_dir / "multistandard.pdf").resolve()),
        "bom.csv": str((output_dir / "bom.csv").resolve()),
        # include the source for convenience if present
        **({src.name: str((output_dir / src.name).resolve())} if src else {}),
    }
