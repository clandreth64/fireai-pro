...
Version: 1.3.2 Master (Production Ready - HARDENED & FIXED NAMING)
License: Proprietary
"""
...
try:
    ...
    def get_or_create(metric_cls, name, documentation, **kwargs):
        ...
        return existing if existing else metric_cls(name, documentation, **kwargs)
except Exception:
    class _Noop:
...
class FallbackBracingEngine:
    ...
    def calculate_bracing(project_data: Dict, routing_result: Any) -> Dict[str, Any]:
        try:
            ...
            seismic_multiplier = {'low': 1.0, 'moderate': 1.2, 'high': 1.5, 'very_high': 1.8}.get(seismic_zone, 1.2)
            ...
            if hasattr(routing_result, 'pipe_segments'):
                for i, segment in enumerate(routing_result.pipe_segments):
                    if i % 3 == 0 and hasattr(segment, 'start_point'):
...
class ProductionAlertManager:
    def __init__(self, config: ProductionConfig, logger: logging.Logger):
        ...
        self.templates = {
            ...
            'fallback_usage': {
                'subject': '⚠️ FireAI Pro - Fallback Usage Alert',
                'template': '''
                <h2>⚠️ FireAI Pro Fallback Usage</h2>
            ...
            'export_failure': {
                'subject': '📄 FireAI Pro - Export Failure',
                'template': '''
                <h2>📄 FireAI Pro Export Failure</h2>
    ...
    async def _send_email_alert(self, alert_type: str, alert_data: Dict[str, Any]):
        if not self.config.SMTP_SERVER or not self.config.ALERT_EMAILS:
            return
        template = self.templates.get(alert_type, self.templates['job_failure'])
        ...
        <body style="font-family: Arial, sans-serif;">
            ...
            <p style="font-size: 12px; color: #666;">
                FireAI Pro Platform - {self.config.ENVIRONMENT.upper()}
            </p>
        </body>
        </html>
        """
    ...
    async def _send_slack_alert(self, alert_type: str, alert_data: Dict[str, Any]):
        ...
        response = requests.post(self.config.SLACK_WEBHOOK_URL, json=slack_message, timeout=10)
    ...
    async def upload_file(self, local_path: str, remote_key: str, content_type: str = None) -> str:
        try:
            ...
            if self.storage_type == "local":
                local_storage_path = Path(self.config.LOCAL_STORAGE_PATH) / Path(remote_key).parent
                local_storage_path.mkdir(parents=True, exist_ok=True)
                target_path = Path(self.config.LOCAL_STORAGE_PATH) / remote_key
                if Path(local_path) != target_path:
                    shutil.copy2(local_path, target_path)
                self.logger.info(f"Local file saved: {target_path}")
...
class FireAIProMasterOrchestrator:
    ...
    def _safe_text(self, path: Path, text: str) -> bool:
        try:
            ...
            self.logger.info(f"Saved text file: {path}")
            return True
        except Exception as e:
            self.logger.error(f"Write failed for {path}: {e}")
            return False

    def _make_dxf(self, path: Path, job_id: str) -> bool:
        try:
            doc = ezdxf.new("R2010")
            msp = doc.modelspace()
    ...
    def _make_pdf(self, path: Path, title: str, body: str) -> bool:
        try:
            ...
            c.showPage()
            c.save()
            self.logger.info(f"PDF generated: {path}")
            return True
        except Exception as e:
    ...
    async def _run_module_with_monitoring(self, module_func, module_name: str, result: ProjectResult, *args, **kwargs):
        ...
        try:
            ...
            output = await module_func(*args, **kwargs) if asyncio.iscoroutinefunction(module_func) else module_func(*args, **kwargs)
    ...
    async def _run_cad_module(self, project_data: Dict, job_dir: Path, job_logger: logging.LoggerAdapter, dry_run: bool):
        if dry_run:
            job_logger.info("DRY RUN: Skipping CAD module")
            return {'status': 'skipped', 'cad_data': {}}
        if CAD_AVAILABLE:
            job_logger.info("Running Enhanced CAD Engine")
    ...
    async def _run_symbols_ai_module(self, project_data: Dict, job_dir: Path, job_logger: logging.LoggerAdapter, dry_run: bool):
        if dry_run:
            job_logger.info("DRY RUN: Skipping SymbolsAI module")
            return {'status': 'skipped', 'symbols': []}
        if SYMBOLS_AI_AVAILABLE:
            job_logger.info(f"Running SymbolsAI ({SYMBOLS_AI_ENGINE})")
    ...
    async def _extract_routing_constraints(self, result: ProjectResult, project_data: Dict, job_logger: logging.LoggerAdapter, nfpa13_constraints: Dict = None) -> RoutingConstraints:
        try:
            if CODES_STANDARDS_AVAILABLE and nfpa13_constraints:
                job_logger.info("Extracting NFPA13 constraints")
    ...
    async def _validate_routing_compliance(self, routing_result: Any, constraints: RoutingConstraints, job_logger: logging.LoggerAdapter) -> ComplianceResult:
        try:
            if CODES_STANDARDS_AVAILABLE:
                job_logger.info("Validating routing compliance")
    ...
    async def _refine_constraints(self, constraints: RoutingConstraints, compliance_result: ComplianceResult, job_logger: logging.LoggerAdapter) -> RoutingConstraints:
        try:
            if CODES_STANDARDS_AVAILABLE:
                job_logger.info("Refining routing constraints")
            ...
            job_logger.warning("Using original constraints due to unavailable Codes & Standards")
            return constraints
        except Exception as e:
    ...
    async def _achieve_compliant_routing(self, result: ProjectResult, project_data: Dict, constraints: RoutingConstraints, job_logger: logging.LoggerAdapter, dry_run: bool, enable_monitoring: bool):
        ...
        for iteration in range(max_iterations):
            try:
                job_logger.info(f"Routing iteration {iteration + 1}/{max_iterations}")
    ...
    async def _extract_exact_pipe_network(self, routing_result: Any, job_logger: logging.LoggerAdapter) -> ExactPipeNetwork:
        try:
            if ROUTING_AVAILABLE:
                job_logger.info("Extracting exact pipe network")
    ...
    async def _run_hydraulics_module(self, pipe_network: ExactPipeNetwork, job_logger: logging.LoggerAdapter, dry_run: bool):
        if dry_run:
            job_logger.info("DRY RUN: Skipping Hydraulics module")
            return {'status': 'skipped', 'hydraulics': {}}
        if HYDRAULICS_AVAILABLE:
            job_logger.info("Running Enhanced Hydraulics Engine")
    ...
    async def _run_bracing_module(self, project_data: Dict, routing_result: Any, job_logger: logging.LoggerAdapter, dry_run: bool):
        if dry_run:
            job_logger.info("DRY RUN: Skipping Bracing module")
            return {'status': 'skipped', 'bracing': {}}
        if HANGING_BRACING_AVAILABLE:
            job_logger.info("Running Enhanced Bracing Engine")
    ...
    async def _run_products_module(self, routing_result: Any, job_logger: logging.LoggerAdapter, dry_run: bool):
        if dry_run:
            job_logger.info("DRY RUN: Skipping Products AI module")
            return {'status': 'skipped', 'products': {}}
        if PRODUCTS_AI_AVAILABLE:
            job_logger.info("Running Master ProductsAI Enhanced")
    ...
    async def _run_multi_standard_validation(self, routing_result: Any, project_data: Dict, job_logger: logging.LoggerAdapter, dry_run: bool):
        if dry_run:
            job_logger.info("DRY RUN: Skipping Multi-Standard validation")
            return {'status': 'skipped', 'validation': {}}
        if CODES_STANDARDS_AVAILABLE:
            job_logger.info("Running Multi-Standard validation")
    ...
    async def _generate_guaranteed_dxf(self, result: ProjectResult, job_dir: Path, job_logger: logging.LoggerAdapter, dry_run: bool) -> str:
        ...
        try:
            if CAD_AVAILABLE:
                job_logger.info(f"Generating DXF with Enhanced CAD Engine: {dxf_path}")
                ...
                job_logger.warning(f"Using DXF fallback: {dxf_path}")
                self._make_dxf(dxf_path, result.job_id)
                job_logger.info(f"✅ DXF generated via fallback: {dxf_path}")
                await self.alert_manager.send_fallback_usage_alert("enhanced_cad_engine", "CAD engine unavailable")
            if not dxf_path.exists():
                job_logger.error(f"DXF file not found after generation: {dxf_path}")
                raise FileNotFoundError(f"DXF file not created: {dxf_path}")
            url = await self.storage_manager.upload_file(str(dxf_path), f"{result.job_id}/design.dxf", "application/dxf")
            job_logger.info(f"DXF uploaded: {url}")
            if self.metrics:
    ...
    async def _generate_guaranteed_ifc(self, result: ProjectResult, job_dir: Path, job_logger: logging.LoggerAdapter, dry_run: bool) -> str:
        ...
        try:
            if ROUTING_AVAILABLE:
                job_logger.info(f"Generating IFC with Routing Advanced: {ifc_path}")
                ...
                job_logger.warning(f"Using IFC fallback: {ifc_path}")
            ...
            if not ifc_path.exists():
                job_logger.error(f"IFC file not found after generation: {ifc_path}")
                raise FileNotFoundError(f"IFC file not created: {ifc_path}")
            url = await self.storage_manager.upload_file(str(ifc_path), f"{result.job_id}/model.ifc", "application/ifc")
            job_logger.info(f"IFC uploaded: {url}")
            if self.metrics:
    ...
    async def _generate_pdf_report(self, pdf_type: str, result: ProjectResult, job_dir: Path, job_logger: logging.LoggerAdapter, dry_run: bool) -> str:
        ...
        try:
            if REPORTLAB_AVAILABLE:
                job_logger.info(f"Generating {pdf_type.upper()} PDF with ReportLab: {pdf_path}")
                ...
                job_logger.warning(f"Using {pdf_type.upper()} PDF fallback: {pdf_path}")
            ...
            if not pdf_path.exists():
                job_logger.error(f"{pdf_type.upper()} PDF file not found after generation: {pdf_path}")
                raise FileNotFoundError(f"{pdf_type.upper()} PDF file not created: {pdf_path}")
            url = await self.storage_manager.upload_file(str(pdf_path), f"{result.job_id}/{pdf_path.name}", "application/pdf")
            job_logger.info(f"{pdf_type.upper()} PDF uploaded: {url}")
            if self.metrics:
    ...
    async def process_project(self, project_data: Dict[str, Any], project_name: str, dry_run: bool = False, export_formats: List[str] = None, job_id: str = None, enable_monitoring: bool = True) -> ProjectResult:
        ...
        try:
            ...
            job_logger.info(f"Job directory created: {job_dir}")
            # PHASE 1: SETUP & VALIDATION
            job_logger.info("🔄 PHASE 1: Setup and validation...")
            ...
            export_timeout = int(os.getenv("EXPORT_TIMEOUT_S", "60"))
            async def export_task(fmt):
                try:
                    ...
                    job_logger.error(f"Export task for {fmt} failed: {e}")
                    result.errors.append(f"Export {fmt}: {e}")
            tasks = [export_task(fmt) for fmt in export_formats]
            await asyncio.wait([asyncio.create_task(t) for t in tasks], timeout=export_timeout)
            # Verify all expected files exist
            expected_files = ['design.dxf', 'model.ifc', 'compliance.pdf', 'hydraulics.pdf', 'bom.pdf', 'bracing.pdf', 'multistandard.pdf']
            missing_files = [f for f in expected_files if f not in [Path(p).name for p in result.export_files.values()]]
            if missing_files:
                job_logger.error(f"Missing expected export files: {missing_files}")
                result.errors.append(f"Missing export files: {', '.join(missing_files)}")
                result.status = JobStatus.PARTIAL
            # STRICT Quality Gate
            if os.getenv("FIREAI_ENABLE_STRICT", "false").lower() == "true":
                bad = [p.name for p in [job_dir / f for f in expected_files] if self._is_low_quality(p)]
                if bad:
            ...
            if self._safe_text(job_dir / "artifacts.json", json.dumps(manifest, indent=2)):
                job_logger.info(f"Artifacts manifest generated: {job_dir / 'artifacts.json'}")
            else:
                job_logger.error("Failed to generate artifacts manifest")
                result.errors.append("Failed to generate artifacts manifest")
            # Finalize
            ...
            if result.status == JobStatus.COMPLETED:
                ...
                await self.alert_manager.send_job_failure_alert(job_id, project_name, f"Job completed with errors: {result.errors}", result)
    ...
    def _is_low_quality(self, path: Path) -> bool:
        try:
            if not path.exists():
                self.logger.warning(f"Quality check: File not found: {path}")
                return True
